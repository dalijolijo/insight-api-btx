...
function BlockController(options) {
  ...
  this._block = this.node.services.bitcoind;
  this._header = this.node.services.bitcoind;
  this._timestamp = this.node.services.bitcoind;
}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self._block.height - blockCached.height + 1;
  ...
  if (summaryCache) {
    ...
    self._block.getRawBlock(hash, function(err, blockBuffer) {
      ...
      self._header.getBlockHeader(hash, function(err, blockHeader) {
        ...
        var confirmations = self._block.height - height + 1;
